...
export function getSkinData({ randomSampling = false, ww = 512, hh = 512, skinnedMesh }) {
  /** @type {SkinnedMesh} */
  let it = skinnedMesh
  ...
  function initData({ src, name = '' }) {
    ...
    for (let px = 0; px < pxAll; px++) {
      ...
      if (randomSampling) {
        let r = Math.floor((i + Math.random() * max) % max)
        if (iSize >= 1) {
          tex.image.data[i * 4 + 0] = attrib.getX(r) || 0
        } else {
          tex.image.data[i * 4 + 0] = 0
        }
        if (iSize >= 2) {
          tex.image.data[i * 4 + 1] = attrib.getY(r) || 0
        } else {
          tex.image.data[i * 4 + 1] = 0
        }
        if (iSize >= 3) {
          tex.image.data[i * 4 + 2] = attrib.getZ(r) || 0
        } else {
